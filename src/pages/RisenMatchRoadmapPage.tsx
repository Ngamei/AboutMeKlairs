...
import { RisenMatchRoadmapSection } from '../components/RisenMatchRoadmapSection';
import { useLanguage } from '../lib/i18n';

export default function RisenMatchRoadmapPage() {
  const { t } = useLanguage();
  const concept = t.concepts.items.risenMatch;
  ...
  return (
    <div className="min-h-screen text-slate-800 font-sans antialiased">
      ...
      <main className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16">
        ...
        <h1 className="text-3xl sm:text-4xl font-bold text-slate-900 tracking-tight mb-3">{concept.title}</h1>
        <p className="text-sm text-slate-500 mb-2">{concept.subtitle}</p>
        <p className="text-sm font-semibold text-slate-800 mb-10">{t.links.mvpRoadmap}</p>

        <div className="bg-white/90 rounded-2xl border border-slate-200/90 p-6 sm:p-8 shadow-card backdrop-blur-sm">
          <RisenMatchRoadmapSection />
        </div>
      </main>
    </div>
  );
